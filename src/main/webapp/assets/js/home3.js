$(document).ready(function(){
	var INDEX=0;
	var BIGSIZE=0;
	BigImages=[{id:"1",path:"images/Extras/c1.jpg"},{id:"3",path:"images/Extras/c3.jpg"},{id:"4",path:"images/Extras/c4.jpg"},{id:"5",path:"images/Extras/c5.jpg"}]

	$("#idlogoutbutton").hide();
	$("#idprofilebutton").hide();
	var buyer_data = checkCookie("Buyer_data");
//	alert(buyer_data);

	if(buyer_data==null || buyer_data.dob==null){
//    	alert();
    	getAllDeals();
    }
    else{
    	var buyer_id = buyer_data.id;
//    	alert(buyer_id);
    	getDealsByBuyerId(buyer_id);
    }
    
//    appendBigImages(BigImages);
    getCategoryList();
...
//	BigImagesIndices=[]
	...
	function appendBigImages(table){
		BIGSIZE=table.length;
		var str=""
		for(var i=0;i<table.length;i++){
			var deal=table[i];
			var id=deal.id;
			var src=deal.path;
//			BigImagesIndices.append(image.id);
            var url = "itemslist.html?deal_id="+id;
			var addition="<a href='"+url+"'><img id='idbigimage_"+i+"'   class='_232NW7 dImZt_ displaynone' src='"+src+"'></a>";
			str+=addition;
		}
		$("#idbigcorosal").empty();
		$("#idbigcorosal").append(str);
		
		//var changeId=BigImagesIndices[INDEX];
		$("#idbigimage_0").removeClass("displaynone");
	}
	
	$("body").on("click",'#idleftcorosalbutton',function () {
		$("#idbigimage_"+INDEX).addClass("displaynone");
		INDEX=(INDEX-1)%(BIGSIZE);
		if(INDEX<0){
			INDEX=BIGSIZE-1;
		}
		//alert(INDEX);
		$("#idbigimage_"+INDEX).removeClass("displaynone");
	});

	$("body").on("click",'#idrightcorosalbutton',function () {
		$("#idbigimage_"+INDEX).addClass("displaynone");
		INDEX=(INDEX+1)%(BIGSIZE);
		//alert(INDEX);
		$("#idbigimage_"+INDEX).removeClass("displaynone");
	...
	/*
	function getDealListWithBuyerId(buyer_id){
	...
	*/
	...
	function getAllDeals(){
		$.ajax({
		    ...
		    success : function (data) {
		    	if(data){
		    		appendBigImages(data);
		    		
		    	}
		    	else{
		    		alert("failed  to get deals");
		    	}
		    }
	...
	function getDealsByBuyerId(buyer_id){
//		alert(price_range);
//		alert(url_api);
...
//			url:"http://localhost:8080/flipkart/webapi/items/getReport",
		    ...
		    success : function (data) {
		    	if(data){
		    		...
		    		alert("failed to get deals for buyerid");
		    	}
		    }
		});
	}	
